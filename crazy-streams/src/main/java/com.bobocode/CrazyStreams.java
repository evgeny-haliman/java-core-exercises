...
public class CrazyStreams {
    ...
    public List<Account> findAccountsByBirthdayMonth(Month birthdayMonth) {
        return accounts.stream()
                .filter(n -> n.getBirthday().getMonth().equals(birthdayMonth))
    ...
    public int getNumOfLettersInFirstAndLastNames() {
        return accounts.stream().mapToInt(n -> n.getFirstName().length() + n.getLastName().length()).sum();
    }
     ...
     *
     * @param emailDomain
     * @return true if there is an account that has an email with provided domaien
     */
        public boolean containsAccountWithEmailDomain(String emailDomain) {
          return   accounts.stream().anyMatch(n-> n.getEmail().contains(emailDomain));
        }
    ...
    public Map<String, BigDecimal> collectBalancesByEmailForAccountsCreatedOn(int year) {
        return accounts.stream()
                .filter(n -> n.getCreationDate().getYear() == year)
